...
from ..inat.client import iter_pages, lookup_taxon
...
async def run_snapshot(species_list: list[SpeciesConfig], place_id: int) -> dict:
    ...
    Streams pages directly to the database to stay within memory limits.
    ...
    for sp in species_list:
        ...
        # Stream pages directly to DB — never hold all observations in memory
        ...
        try:
            async for batch in iter_pages(
                ...
                await db.commit()
                logger.info(f"  Stored {count} observations so far for {taxon_actual_name}")

            logger.info(f"Stored {count} total observations for {taxon_actual_name}")
        finally:
            await db.close()
